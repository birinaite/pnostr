// Fetch translations and relays data
async function loadData() {
  try {
    const [translationsResponse, relaysResponse] = await Promise.all([
    ...
    Object.keys(configData).forEach((lang) => {
      if (config[lang]) {
        config[lang] = { ...config[lang], ...configData[lang] };
      } else {
        config[lang] = configData[lang];
      }
    });

    return { config, relaysData };
  } catch (error) {
    console.error("Failed to load configuration or relays:", error);
    throw error;
  }
}
...
// Main script
loadData()
  .then(({ config, relaysData }) => {
    const relays = relaysData.relays.map((relay) => relay.url);
    ...
    function createMediaPlayer(blob, metadata) {
    ...
    async function handleLoad() {
      ...
      try {
        ...
        createMediaPlayer(blob, metadata);
    ...
    window.onload = function () {
      ...
      if (urlId) {
        ...
        handleLoad(urlId); // Load immediately without setTimeout
      }
    };
  })
  .catch((error) => {
